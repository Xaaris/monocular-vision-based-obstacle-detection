...
def increment_and_get_id_count() -> int:
    global ID_COUNT
    ID_COUNT = ID_COUNT + 1
    return ID_COUNT

def _average_descriptor_distance(descriptor_a, descriptor_b) -> float:
        ...
        if len(self.occurrences) >= 2:
            ...
            for i in range(len(last_n_instances) - 1):
                ...
                if current is not None and current.descriptors is not None and last is not None and last.descriptors is not None:
                    matches = MATCHER.match(current.descriptors, last.descriptors, mask=None)
...
class DetectedObjects:
    ...
    def add_object(self, new_obj_instance, verbose=False):
        ...
        if highest_similarity > SAMENESS_THRESHHOLD:
            ...
            if verbose:
                print(f"New Object!")
            new_obj_track = ObjectTrack(increment_and_get_id_count(), [new_obj_instance])
